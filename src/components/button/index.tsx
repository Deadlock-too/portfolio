import clsx from 'clsx'
import { MdArrowOutward } from 'react-icons/md'
import Link from 'next/link'

type ButtonProps = {
  url: string
  label: string
  showIcon?: boolean
  target?: string
  className?: string
}

export default function Button({ url, label, showIcon = true, target = '_blank', className }: ButtonProps) {
  return (
    <Link
      className={clsx(
        'group relative flex w-fit items-center justify-center overflow-hidden rounded-md border-2 border-slate-900 bg-slate-50  px-4 py-2 font-bold transition-transform ease-out  hover:scale-105',
        className,
      )}
      href={url}
      target={target}
    >
      <span className='absolute inset-0 z-0 h-full translate-y-9 bg-yellow-500 transition-transform duration-300 ease-in-out group-hover:translate-y-0' />
      <span className='relative flex items-center text-black justify-center gap-2'>
        {label} {showIcon && <MdArrowOutward className='inline-block' />}
      </span>
    </Link>
  )
}
