      ...
      <CardContainer description={ description } className="md:grid-cols-2">
        {
          songs.map((song, index) => (
            <Card
              key={ index }
              className={ clsx('border-2 song-card flex flex-row p-4 -ml-6 gap-2 hover:opacity-100 hover:scale-105 transition-all ease-in-out duration-300',
                playing === index ? 'bg-black/80 opacity-80 scale-105' : 'bg-black/60 opacity-60'
              ) }
            >
              <div className="song-card-index">
                <p
                  className="text-sm bg-black text-center content-center border-2 rounded-[50%] border-white w-6 h-6 -ml-2 -mt-2 text-slate-100"
                >{ index + 1 }</p>
              </div>
              <div
              ...
              >
                ...
                <div
                  className={ clsx('song-card-bar bg-yellow-500 h-1.5 w-full ',
